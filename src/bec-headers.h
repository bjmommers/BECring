...
vector<double> initialisethreemode
...

{
    ...
    //calculate the ring radius, and therefore the oscillation time
    radius = (xmax-xmin)/(2.0*PI);
    cout << "Ring radius (sim units): " << radius << endl;
    oneosc = PI/((imm*imm)/(4.0*radius*radius));//2.0*radius*radius

    //determine the number of full oscillations in the sim, size results vector
    numosc = floor(tmax/oneosc);
    cout << "Number of oscillations: " << numosc << endl;
    results.resize(numosc);
    timevec.resize(numosc);

    cout << "phasevec[0] = " << phasevec[0] << endl;
    ...
    for(j=0;j<numosc;j++)
    {
        ...
        //results[j] = phasevec[(int) (((j+1)*oneosc)/tstep)] - phasevec[(int) ((j*oneosc)/tstep)];
        timevec[j] = (j+1)*oneosc;
        cout << "time: " << timevec[j] << endl;
        cout << "phasevec[" << (int) (timevec[j]/tstep) << "]: " << phasevec[(int) (timevec[j]/tstep)] << endl;
        results[j] = phasevec[(int) (timevec[j]/tstep) + 1]/(PI*timevec[j]);
    }
...

{
    ...
    radius = (xmax-xmin)/(2.0*PI);
    cout << "Ring radius (sim units): " << radius << endl;
    oneosc = PI/((imm*imm)/(4.0*radius*radius) - g*a0*a0);//2.0*radius*radius
