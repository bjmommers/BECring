...
*/    
    ...
    statsmatrix.resize(((unsigned int) aggstatvec.size())*Nt);
    istatsmatrix.resize(((unsigned int) aggstatvec.size())*Nit);
    ...
    else if(threemode > 0) //one-component only for now
    {
        ...
        initialisethreemode(zoldpsi1,x,tmphase,bgmode,modprop,modebias,imm,omegaimag,imprintamp,res,a11);
        for(j=0;j<Nx;j++)
        {
...
/**************************END CONTROL LOOP************************************/
    ...
    for(k=0;k<Nt;k++)
    {
        for(j=0;j<(int) aggstatvec.size()-1;j++)
        {
            istats << setw(8) << statsmatrix[(k*(int) aggstatvec.size())+j] << "\t";
        }
        istats << setw(8) << statsmatrix[(k*(int) aggstatvec.size())+((int) aggstatvec.size()-1)] << endl;
    }
...
//    spacetimefile << setw(12);
    ...
    for(j=0;j<Nx;j++)
    {
        spacetimefilelabelled << densitytime[((Nx-1)*Nx)+j] << "\t";
    }
        spacetimefilelabelled << densitytime[((Nx-1)*Nx)+(Nx-1)];
    ...
    col2vec(phasevec,phasesingle,nfreq+1,1,0);


    rotationmeasurement = strobo_rot_measurement(phasevec,timevec,tstep,tmax,xupper,xlower,omegareal,imm);
